          ...
          if user_id:
              ...
              for tweet in tweets:
                  tweet_data = {
                      ...
                      "is_seeker": is_seeker_related(tweet["text"])
                  }

                  # Seeker 관련 트윗 수집
                  if tweet_data["is_seeker"]:
                      seeker_tweets.append(tweet_data)

                  # @solanamobile 최근 트윗 수집 (백업용)
                  if username == "solanamobile":
                      solanamobile_tweets.append(tweet_data)

      # Seeker 트윗이 있으면 사용, 없으면 solanamobile 최근 트윗 사용
      if seeker_tweets:
          print(f"Found {len(seeker_tweets)} Seeker-related tweets")
          return seeker_tweets
      else:
